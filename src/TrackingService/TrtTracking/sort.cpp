...
public:
  Sort(int max_age, int min_hits, float iou_threshold)
      : m_max_age(max_age), m_min_hits(min_hits),
  ...
  Update(const std::vector<DetectionBox> &dets) override;
  ~Sort() { ms_num_session--; };
...
static void AssociateDetectionsToTrackers(
    const std::vector<DetectionBox> &dets, const std::vector<TrackingBox> &trks,
    float iou_threshold, std::vector<std::vector<int>> &matches,
  ...
  // cout << iou_matrix.size() << endl;
  // for (auto &i : iou_matrix) {
  //   for (auto &j : i) {
  //     cout << j;
  ...
  // cout << iou_matrix << assignment << endl;
...
std::vector<TrackingBox> Sort::Update(const std::vector<DetectionBox> &dets) {
  ...
  AssociateDetectionsToTrackers(dets, trks, m_iou_threshold, matches,
